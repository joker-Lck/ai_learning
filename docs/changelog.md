...
### 新增 — SM-2 间隔重复系统
- **错题复习调度**：基于 SM-2 算法的遗忘曲线间隔重复
  - `error_notes` 表新增字段：`review_count`、`last_review`、`next_review`、`ease_factor`、`review_interval`
  - 新错题自动设置 `next_review = 明天`
  - 做对延长间隔（1→3→7→17→42天），做错重置为1天
- **新增端点**：
  - `POST /review-submit` — 提交复习结果，更新间隔重复调度
  - `GET /review-due` — 基于 `next_review` 查询待复习错题（替代原有 `created_at` 算法）
- **首页待复习提醒**：工作台显示待复习错题数量，点击直接进入复习模式
- **数据库迁移脚本**：`scripts/migrate_error_notes_sr.py`

### 新增 — 题库复用
- **question_bank 表**：AI 出过的题自动存入题库，后续出题可复用
- **智能混合出题**：题库最多占 60%，剩余由 AI 生成，确保用户总能遇到新题
- **题库去重**：同一学科下相同题目文本不重复存入
- **使用次数追踪**：优先取使用次数少的题，平衡题库消耗

### 新增 — 做题答错自动保存错题本
- 在线做题答错的题目自动保存到错题本（去重）
- 从 quiz_sessions 获取学科信息，自动填充错题字段

### 新增 — 错题巩固功能
- 错题本中可选择学科和题量，直接进入做题模式重做错题
- 做题模块"错题重练"模式改为从错题本直接取题（不再 AI 重新生成）
- 支持从错题本 URL 参数自动进入复习模式

### 新增 — 做题实时计分板
- 做题过程中实时显示已答数、正确数、错误数、正确率
- 正确率颜色随分数变化（≥80% 绿色、≥60% 黄色、<60% 红色）

### 新增 — 流式路径生成
- 学习路径生成改为 SSE 流式输出（打印机效果）
- 生成过程中实时显示 AI 输出内容

### 优化 — 语音输入错误提示
- 语音输入失败时显示可见的错误提示（麦克风权限、浏览器不支持等）
- 错误提示 5 秒后自动消失

### 移除 — 学生画像精简
- 移除学生画像中的"成绩"Tab（成绩数据保留，仅移除 UI 入口）
- 移除学生画像中的"错题本"Tab（错题数据保留，仅移除 UI 入口）
- 学生画像现在只保留"画像"和"课程表"两个 Tab
- 移除番茄钟功能

---

## v8.4.0 (2026-07-27) — 学习行为追踪与模块合并

### 新增 — 学习行为追踪
- **视频浏览记录**：点击视频时自动记录标题、标签、分类到活动日志
- **图书浏览记录**：点击国图/鸠摩链接时记录标题、标签、来源
- **工作台活动流**：新增「视频学习」和「图书查阅」活动类型展示
- **数据存储**：localStorage 存储，按用户名隔离，最多保留 100 条

### 优化 — 学习路径与规划合并
- **去掉 Tab 切换**：学习路径和规划合并为单一页面
- **一键同时生成**：输入学习目标后同时生成路径和规划
- **统一输入框**：支持快捷预设（周计划/备考/新技能）

### 优化 — 脚本重命名
- `scripts/init_databases_v7.2.py` → `scripts/init_databases.py`，移除版本号
- 同步更新 README、CI workflow、deployment 文档、测试、setup.bat、launcher.py 中的所有引用

### 优化 — 学习资源模块三标签页
- AI 资源生成 — 多智能体生成 7 种类型资源
- 视频资源 — Bilibili 学习视频搜索与推荐（带 WBI 签名真实搜索）
- 图书资源 — 国家图书馆 + 鸠摩搜书图书搜索

---

## v8.3.0 (2026-07-27) — 图书资源整合（国家图书馆 + 鸠摩搜书）

### 新增 — 图书资源模块
- **BookResourceModule 组件**
  - 精选图书数据库：人工智能、NLP、大模型、计算机科学、数据分析、前端开发 6 大分类
  - 图书卡片展示：封面、标题、作者、出版社、年份、简介、标签
  - 外部链接跳转：国家数字图书馆 + 鸠摩搜书
  - 分类浏览和关键词搜索
- **后端 API**
  - `GET /api/agent/books/search` — 图书搜索（标题/作者/标签模糊匹配）
  - `GET /api/agent/books/recommend` — 推荐图书（按分类筛选）
  - `GET /api/agent/books/cover` — 封面图片代理（支持豆瓣 CDN）
- **前端 API 客户端**
  - `searchBooks()` — 搜索图书
  - `getBookRecommend()` — 获取推荐图书
- **学习资源模块三标签页**
  - AI 资源生成 — 多智能体生成 7 种类型资源
  - 视频资源 — Bilibili 学习视频搜索与推荐
  - 图书资源 — 国家图书馆 + 鸠摩搜书图书搜索
