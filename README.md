...
系统在**首页工作台**和**学生画像页面**展示 6 维度学习能力雷达图，基于画像数据动态评估：

| 维度 | 评分依据 | 分值范围 | 默认值 |
|------|---------|---------|--------|
| 知识基础 | `knowledge_base.level` 映射（入门=1, 初级=2, 中级=3, 高级=4, 精通=5）；或根据知识点数量推断 | 1-5 | 3 |
| 学习目标 | `learning_goals` 数组长度（目标越明确分数越高） | 1-5 | 3 |
| 记忆能力 | `learning_history` 长度（历史记录越多，记忆能力越强） | 1-5 | 3 |
| 自控力 | `preferred_resources` 中是否包含计划性偏好（计划/规划/定时） | 1-5 | 3 |
| 专注度 | `interest_areas` 参与度（兴趣领域越多越专注） | 1-5 | 3 |
| 学习深度 | `interest_areas` 广度（>3个领域=4分, >6个=5分）；`weak_points` 过多则降分 | 1-5 | 3 |

**展示位置**：
- 首页工作台右侧边栏（迷你版 200px）
- 学生画像页面顶部（完整版 280px + 分数标签网格）

**技术实现**：
- 使用 Recharts `RadarChart` 组件
- 紫色主题（`#a78bfa`）匹配系统 UI
- 数据变化时自动重新计算（`useMemo` 缓存）
