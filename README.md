...
系统在**首页工作台**和**学生画像页面**展示 6 维度学习能力雷达图，支持**两种评估模式**：

#### 评估模式

| 模式 | 触发方式 | 数据来源 | 准确度 |
|------|---------|---------|--------|
| **规则评估** | 自动（默认） | 基于画像字段的规则映射 | 基础 |
| **AI 评定** | 点击「AI 评定」按钮 | MiMo 综合分析画像 + 使用数据 | 高 |

#### 6 维度评分标准

| 维度 | 规则评估依据 | AI 评定依据 | 分值 |
|------|------------|------------|------|
| **知识基础** | `knowledge_base.level` 映射：入门=1, 初级=2, 中级=3, 高级=4, 精通=5 | 综合知识掌握程度、学习历史深度、资源使用广度 | 1-5 |
| **学习目标** | `learning_goals` 数组长度：0个=3, 1个=4, 2个+=5 | 目标明确度、可执行性、与当前水平的匹配度 | 1-5 |
| **记忆能力** | `learning_history` 长度：>10条=4, >20条=5 | 短期/语义/情景记忆条数、记忆保持率、遗忘曲线状态 | 1-5 |
| **自控力** | `preferred_resources` 含计划类关键词=4, >=3个偏好+1 | 学习频率、计划执行率、资源使用规律性 | 1-5 |
| **专注度** | `cognitive_style` 含深度/专注=4 | 学习时长分布、单次学习深度、中断频率 | 1-5 |
| **学习深度** | `interest_areas` >3个=4, >6个=5; `weak_points` 多则降分 | 兴趣领域广度、薄弱环节改善率、知识图谱复杂度 | 1-5 |

#### AI 评定流程

```
用户点击「AI 评定」→ 系统收集画像 + 使用数据
→ MiMo 综合分析 → 输出 6 维度分数 + 评估理由
→ 保存到 profile_evaluations 数据库表
...
| 数据 | 来源 | 说明 |
|------|------|------|
| 学习资源数 | `learning_resources` 表 | 生成的资源总量 |
| 学习活动数 | `learning_activities` 表 | 学习行为记录 |
| 短期记忆条数 | `short_term_memory` 表 | 对话上下文积累 |
| 语义记忆条数 | `semantic_memory` 表 | 知识点记忆 |
| 情景记忆条数 | `episodic_memory` 表 | 学习场景记忆 |
| 实体记忆条数 | `entity_memory` 表 | 知识图谱实体 |

#### 展示位置

- **首页工作台**：右侧边栏（迷你版 200px），带「AI 评定」按钮
- **学生画像页面**：顶部（完整版 280px + 分数标签网格）

#### 评判标准说明

- **1 分**：该维度几乎无数据支撑，处于初始状态
- **2 分**：有少量数据，但表现较弱
- **3 分**：中等水平，有一定数据支撑（默认值）
- **4 分**：良好，数据充分且表现积极
- **5 分**：优秀，数据丰富且持续稳定

**技术实现**：
- 共享计算函数：`frontend/lib/radar.ts`
- Recharts `RadarChart` 组件，紫色主题（`#a78bfa`）
- AI 评分缓存：`localStorage`（24小时 TTL）
- 评定记录持久化：`profile_evaluations` 表（ai_memory.db）
- API 端点：`POST /api/agent/evaluate-profile`
...
学习资源模块分为「AI 资源生成」、「视频资源」和「图书资源」三个标签页。
...
**功能特性**：
- **真实搜索**：带 WBI 签名的 Bilibili API，支持任意关键词搜索
...
- **行为追踪**：记录用户观看的视频类型，用于学习情况分析

#### 图书资源（国家图书馆 + 鸠摩搜书）

集成国家数字图书馆和鸠摩搜书，提供图书搜索和推荐。

**功能特性**：
- **图书搜索**：基于精选书库的标题、作者、标签模糊匹配
- **分类浏览**：人工智能、NLP、大模型、计算机科学、数据分析、前端开发
- **图书卡片**：封面图、标题、作者、出版社、年份、简介、标签
- **外部链接**：一键跳转国家图书馆或鸠摩搜书查看完整信息
- **行为追踪**：记录用户查阅的图书类型和来源，用于学习情况分析
- **热门搜索**：预置 Python教程、机器学习、深度学习等快捷标签

### 4. 学习路径与规划模块

MiMo 分析学生当前知识水平，同时生成学习路径和学习计划。
...
**新增**：
- 今日建议升级为专业学习规划师：多维度薄弱点分析 + 遗忘曲线复习推荐 + 认知风格策略
- 智能提醒系统：基于遗忘曲线（1/3/7/14/30 天）的错题复习推送 + 学习计划检查
- 成绩趋势折线图：Recharts LineChart 多科目趋势可视化
...
**新增 API**：
- `GET /api/agent/review-due` — 待复习错题（遗忘曲线调度）
- `GET /api/agent/review-reminder` — 综合学习提醒
