...
> 采用 MiMo 大模型驱动，集成 RAG 检索增强、记忆系统、防幻觉机制
...
[![MiMo](https://img.shields.io/badge/AI_MiMo-v2.5-purple.svg)](https://api.mimo.ai/)
...
本系统采用**多智能体协同架构**和**多数据库设计**，基于 **MiMo 大模型** 为高等教育学生提供**个性化学习资源生成**服务。

### 核心优势

- **MiMo 大模型驱动** — 基于 MiMo-v2.5-pro 推理模型，支持文本对话、视觉理解、图片生成、语音合成
...
| 能力 | 模型 | 说明 |
|------|------|------|
| **文本推理** | `mimo-v2.5-pro` | 主力推理模型，支持复杂逻辑、代码生成、数学推导 |
| **视觉理解** | `mimo-v2.5` | 多模态视觉模型，支持图片识别、OCR、图表理解 |
| **图片生成** | `mimo-image` | 文生图，生成教学示意图、知识点图解 |
| **语音合成** | `mimo-tts` | 文字转语音，生成音频讲解 |
| **文本向量化** | `mimo-embedding` | 768 维向量嵌入，用于 RAG 语义检索 |
...
### 8. 混合检索系统（ANN + KNN + RRF）

- **KNN 关键词检索** — SQLite FTS5 全文索引精确匹配专业术语
- **ANN 向量检索** — FAISS 余弦相似度语义匹配（基于 mimo-embedding 768维向量）
...
- Python 3.14
- Node.js 18+
- MiMo API Key（[申请地址](https://api.mimo.ai/)）
...
# 编辑 .env，填写以下配置：
# - MIMO_API_KEY（必需，MiMo API 密钥）
# - MIMO_BASE_URL（可选，默认 https://api.mimo.ai/v1）
...
```
┌─────────────────────────────────────────────────────────────────┐
│  前端 (Next.js)  ·  移动端 (React Native + Expo)                │
│  TypeScript + Tailwind CSS + Zustand + Framer Motion            │
...
- RAG 检索增强（11 种策略路由）
- 向量相似度检索（FAISS IndexFlatIP + mimo-embedding 768维）
- 防幻觉机制（RAG 验证 + 事实核查 + 引用标注 + 交叉验证）
- MiMo-v2.5-pro 推理模型
- MiMo-image 图片生成
- MiMo-tts 语音合成
- MiMo-v2.5 视觉理解（OCR + 多模态）
...
CREATE TABLE knowledge_documents (
    ...
    embedding TEXT,                -- JSON: mimo-embedding 向量嵌入
    uploaded_by INTEGER,
    usage_count INTEGER DEFAULT 0,
...
**数据管理**（画像模块内 Tab 切换）：
- **课程表**：手动录入/编辑/删除 + 文件导入（PDF/Word/Excel/PPT/图片），MiMo 自动识别
- **成绩管理**：录入 + 文件导入，按学期统计
- **错题本**：添加/标记掌握/删除 + 文件导入
- **学习计划**：MiMo 生成 + 手动创建
...
**使用流程**：
1. 进入辅导模块，MiMo 自动发送欢迎引导消息
2. 选择学科
3. 输入问题
4. MiMo 检索 RAG 知识库 + 用户记忆上下文
5. 生成回答（含图解、示例）
...
3. 可选填写学科标签
4. MiMo 自动解析文档、提取知识点、生成摘要
5. 文档入库后通过混合检索引擎检索

**检索能力**：
- **KNN 关键词路径**：SQLite FTS5 全文索引精确匹配专业术语
- **ANN 向量路径**：FAISS + mimo-embedding 语义匹配相近表达
- **RRF 融合排序**：两条路径结果统一排序
...
- MiMo-v2.5-pro 推理模型驱动全部智能体
- MiMo-v2.5 视觉理解（OCR + 图表识别）
- MiMo-image 图片生成（教学示意图）
- MiMo-tts 语音合成（音频讲解）
- mimo-embedding 768 维向量检索
...
用户查询
   ...
   │                       (专业术语精确匹配)
   │
   ├── ANN 向量路径 ──→ mimo-embedding(768维) ──→ FAISS ANN 检索 ──→ Top-K 结果
   │                                                    │
   │                                             三级回退策略
   │                                             ┌──────────────┐
   │                                             │              │
...
| 参数 | 值 | 说明 |
|------|------|------|
| **API** | MiMo Embedding API | `base_url: https://api.mimo.ai/v1` |
| **向量维度** | `768` 维 | 空文本返回 `[0.0] * 768` 零向量 |
| **文本截断** | `8000` 字符 | 超长文本自动截断，防止 API 超时 |
| **鉴权方式** | API Key | MiMo API Key |
...
| `ann` | ANN 向量检索（FAISS + mimo-embedding） | 模糊语义查询 |
...
在智能辅导场景中，系统会先用 KNN 检索历史相似问题，命中则直接返回已有回答，避免重复调用 MiMo 模型。
...
│
├── services/                 # 业务逻辑
│   ├── spark_client.py       # MiMo AI 客户端（OpenAI兼容）
...
│   ├── embedding_service.py  # 向量化服务（mimo-embedding 768维）
...
**A**: 访问 [MiMo 开放平台](https://api.mimo.ai/) 注册账号，在控制台创建 API Key，填入 `.env` 文件的 `MIMO_API_KEY` 字段。

### Q2: 如何体现"多智能体"？

**A**: 系统有 6 个专业智能体（画像/资源/路径/辅导/评估/协调），通过事件驱动消息总线分工协作，不是单一 MiMo 调用。
